...
import { GameItem, GameItemStatus } from "../models/game";
...
export class GameController{
    ...
    constructor(items:GameItem[],
        ...
        renderResetButton(rootElement:HTMLElement):void{
            let button: HTMLButtonElement = rootElement.querySelector('buttoon#reset') as HTMLButtonElement;
            if(button){
        ...
        renderGameBoard():void{
            this.shuffle();
            let boardDiv : HTMLElement = this.element.querySelector('#board');
            if(boardDiv){
                this.items.forEach(it =>{
        ...
        isMatched(id:number,imgElement:HTMLImageElement):boolean{
            ...
            if(openedItems.length==2){
                let checkedMatchedFilter = openedItems.filter(item=>item.id==id);
